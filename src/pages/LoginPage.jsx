...
export default function Login() {
  const { isAuthenticated, login } = useAuth(); // Get the authentication status and login function
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post('https://raotory.com/apis/login.php', {
        email,
        password
      });
    ...
    <div className="flex min-h-full flex-1 flex-col justify-center items-center px-6 py-12 lg:px-8">
      <div className="mt-20 sm:mx-auto sm:w-full sm:max-w-lg border border-gray-200 p-6 rounded-xl">
        ...
        <p className="mt-10 text-center text-sm text-gray-500">
          ...
          <Link to={'/register'} className="pl-1 font-semibold leading-6 text-[#0E90DA] hover:text-indigo-500">
            Register
          </Link>
        </p>
      </div>
